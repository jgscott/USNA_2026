...
## In toy problems, we can do this by hand

For the landing example, we could see the right feature:

- distance from the center
- or equivalently $x_1^2 + x_2^2$
...
{{< video https://www.youtube.com/embed/PAOKOAtti1A?start=17&end=105
    title="Muppet Christmas Carol twenty questions scene"
    width="100%"
    height="80%"
>}}
...
We have _embedded_ our original space $\mathcal{X}$ into the binary cube $\{0,1\}^N$.
...
So basic math operations push us outside $\{0,1\}^N$.  We'd prefer a space that supports both _meaning_ and _math_.
:::
...
We need a fixed set of, say, 500 questions that is sufficiently rich to encode the meaning of _all_ words.
...
- He wiped a streak of ___ off the corner of his mouth.
- Could you pass the ___ for the fries?
- The burger came with lettuce, onion, pickles, and ___.
- He got ___ all over the plate when he squeezed the bottle too hard.
- There was a half-empty bottle of ___ next to the salt and pepper.
...
- Does it often appear in the context **pass the** ___?
...
- look at nearby words in real sentences: the _context window_
- train a model to predict a missing or next word: "Could you pass the ___ for the fries?"
...
Input:
the 10 nearby words, each represented by a vector in $\mathbb{R}^{100}$

Output:
50,000 scores, one for each possible word in the vocab.
